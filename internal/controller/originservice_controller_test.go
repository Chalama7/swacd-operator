...
var _ = Describe("OriginService Controller", func() {
	Context("When reconciling a resource", func() {
		...
		originservice := &swacdv1alpha1.OriginService{}

		BeforeEach(func() {
			By("creating the custom resource for the Kind OriginService")
			err := k8sClient.Get(ctx, typeNamespacedName, originservice)
			if err != nil && errors.IsNotFound(err) {
				resource := &swacdv1alpha1.OriginService{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance OriginService")
